...
function align_all(q::Int, L::Int, filename_par::String, filename_full::String, inspen_file::String, μext::Real, μint::Real;
                delta::Int = 20,
                typel::Symbol = :bm,
    ...
    if filename_align == "" || filename_ins == ""
        ...
        al = enveloptoalign(filename_full, filename_align, filename_ins, delta = delta, ctype = ctype);
    end
...
for k in 1:M
    ...
    N = length(fullseq)
    T0 = true
    res=palign(seq, β.*J, β.*H, β.*lambda_o, β.* lambda_e, ctype = ctype ,mindec = mindec, nprint=nprint, maxiter=maxiter, T0 = T0,  μint = β * μint,seed=seedrng, damp=damp, μext= β * μext, epsconv=epsconv ,verbose=verbose);
    P = res[3].pbf.P
    ...
    if !typedic
        print_results(file_flag, seqpa0, seqpo0, β .* J, β.* H, L, ctype, β * lambda_o, β *  lambda_e, β *μext, β * μint, seqtest, seqins, b0, e0, start, name, sat, β, T0)
    end
    ...
    if enT < enT0
        @printf(file_out, ">%s/%i-%i\n",name, b + parse(Int64,start)-1, e + parse(Int64, start)-1)
        @printf(file_out, "%s\n",seqpa)
    else
        @printf(file_out, ">%s/%i-%i\n",name, b0 + parse(Int64,start)-1, e0 + parse(Int64, start)-1)
        @printf(file_out, "%s\n",seqpa0)
    end
    flush(file_out)
    ...
    if !typedic
        print_results(file_flag, seqpa, seqpo, β .* J, β.* H, L, ctype, β * lambda_o, β *  lambda_e, β *μext, β * μint, seqtest, seqins, b, e, start, name, sat, β, T0)
    end
